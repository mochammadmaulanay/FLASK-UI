...
def index():
    ...
    stats = {
        ...
        'rev_table' : playstore.groupby(['Category','App']).agg({'Reviews':'sum','Rating' :'mean'}).sort_values('Reviews',ascending=False).reset_index().head(10).to_html(classes=['table thead-light table-striped table-bordered table-hover table-sm'])
    }
    ...
    age_range =pd.crosstab(index=playstore['Content Rating'],columns='Jumlah').sort_values('Jumlah',ascending=False).reset_index()

    height = age_range['Jumlah']
    bars = age_range['Content Rating']
